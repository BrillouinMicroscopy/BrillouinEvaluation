...
FilePath = 'BrillouinTestData.h5';

%% set parameter for data analysis

background = 0.4;       % [ ]   maximum background intensity
gap = 10;               % [pix] minimum x and y distance of maxima to the edges of the image
cut = 18;               % [pix] number of quadratic layers around the maxima to cut out in maxima searching progress
ExpMaxima = 2;          % [ ]   expected maxima in each spectrum image

plane_width = 5;        % [pix] width of the interpolated area to get the intensity distibution
...
for n = 1:1:resolution.X
    for m = 1:1:resolution.Y
        ...
        for k = 1:1:size(img_data, 3)
            ...
            maxima = GetMaxima(img_data(:, :, k), background, cut);
