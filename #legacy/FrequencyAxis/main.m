...
ExpMaxima = 4;          % [ ]   expected maxima in each spectrum
...
x0Start = 0.001;
xsStart = 1.2;
VIPAstart = [dStart, nStart, thetaStart, F, x0Start, xsStart];
...
%     [params, ~, ~, FittedCurve] = nfit_4peaks(xLorentzFit, xData, start, floorb);
    ...
    [peaks, peakInt, fwhm, FittedCurve] = IntDistLorentzFit(intensity_sum.', fwhm, SearchBackground*PlaneNum, cut, '4peak');
    peaks = peaks * PixelSize;

    VIPAparams = FitVIPA(peaks, StartOrder, lambda0, bShiftM, VIPAstart, IterNum);
...
[~, m] = VIPApeaks( d, n, theta, lambda0, F, x0, xs, RPeakNumber, StartOrder);
m = sort(m, 'descend');
...
diff1 = c/lambda0 - c./lambda1;
diff2 = c/lambda0 - c./lambda2;
...
ShiftAS = ASCorrected - c/lambda0;

StCorrected = St - drift*xSt;
ShiftSt = StCorrected - c/lambda0;
...
[ xTick, ~, TickLabel ] = GetFrequencyAxisLabel( xmin, xmax, bin, 'GHz', m(1), d, n, theta, F, x0, xs, lambda0);
xTick = xTick/6.5e-6;
...
xlabel('time [s]')
ylabel('frequency shift [GHz]')
plot(xAS, (ASCorrected-c/lambda0)/1e9, '.')
...
xlabel('time [s]')
ylabel('frequency shift [GHz]')
plot(xSt, (StCorrected-c/lambda0)/1e9, '.')
