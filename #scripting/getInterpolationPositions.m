function [positions] = getInterpolationPositions(img, params, width, varargin)
%% parse input values
    p = inputParser;
...
%% plot
    ...
    plot(transpose(borders.x), transpose(borders.y), 'color', 'red', 'linestyle', '--', 'linewidth', 2);
%     plot(plotBorders.x, centers.yOuter, 'color', 'red', 'linestyle', '--', 'linewidth', 2);
    plot(positions.x, positions.y, 'color', 'yellow');
    axis equal
