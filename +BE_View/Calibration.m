...
function plotData(handles, model)
    ...
    if ~isempty(data);
        ...
        if ~isempty(model.parameters.calibration.values_mean.d) && ~isnan(model.parameters.calibration.values_mean.d)
            ...
            wavelength = getWavelength(model.parameters.constants.pixelSize * peaksMeasured, model.parameters.calibration.values_mean, model.parameters.constants, 1);
            peaksMeasured = 1e-9*getFrequencyShift(wavelength, model.parameters.constants.lambda0);
            wavelength = getWavelength(model.parameters.constants.pixelSize * peaksFitted, model.parameters.calibration.values_mean, model.parameters.constants, 1);
            peaksFitted = 1e-9*getFrequencyShift(wavelength, model.parameters.constants.lambda0);
